...
class SMPLlayer(nn.Module):
    ...
    def forward(self, poses, shapes, Rh=None, Th=None, expression=None, 
        ...
        only_shape=False, pose2rot=True, return_transforms=False, **kwargs):
        ...
        if return_verts or not self.use_joints:
            ...
            vertices, joints, Xforms = self.lbs(shapes, poses, v_template,
                                self.shapedirs, self.posedirs,
                                self.J_regressor, self.parents,
                                self.weights, pose2rot=pose2rot, dtype=self.dtype, only_shape=only_shape,
                                use_pose_blending=self.use_pose_blending, use_shape_blending=self.use_shape_blending,
                                J_shaped=self.J_shaped)
            if not self.use_joints and not return_verts:
                vertices = joints
        else:
            vertices, joints, Xforms = self.lbs(shapes, poses, self.j_v_template,
