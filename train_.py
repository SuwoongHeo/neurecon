                ...
                for (indices, model_input, ground_truth) in dataloader:
                    ...
                    if i_val > 0 and int_it % i_val == 0:
                        with torch.no_grad():
                            ...
                            # Sanity check
                            # (v1, v2, v3) = valloader.dataset.__getitem__(21)
                            # r_o, r_d, s_i = rend_util.get_rays(
                            #     v2['c2w'].to(device).unsqueeze(0), v2['intrinsics'].to(device).unsqueeze(0),
...
if __name__ == "__main__":
    ...
    # standard configs
    parser.add_argument('--config', type=str, default='config_test/neusseg_nomask.yaml', help='Path to config file.')
