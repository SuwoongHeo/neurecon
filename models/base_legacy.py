...
class MLPNet(nn.Module):
    def __init__(self, in_dim, out_dim,
        ...
        if len(skips) > 0:
            indims[self.skips] = W + in_dim # W
            outdims[self.skips - 1] = W #W - in_dim #todo if worse performance, turn it back (Weird performance...)

        if len(W_up) > 0:
            outdims[self.W_up] = W * 2
            indims[self.W_up - 1] = W * 2
...
class ImplicitSurface(nn.Module):
    def __init__(self,
        ...
        self.mlpnet = MLPNet(in_dim=input_ch, out_dim=1 + W_geo_feat if W_geo_feat > 0 else 1,
        ...
        #     # decide out_dim
        #     if l == D:
        #         out_dim = 1 + W_geo_feat if W_geo_feat > 0 else 1
        #     elif l in W_up:
        #         out_dim = W*2
        #     elif (l+1) in self.skips and input_feat==0:
        #         out_dim = W - (input_ch+input_feat)  # reduce output dim before the skips layers, as in IDR / NeuS
        #     else:
        #         out_dim = W
        #
        #     # decide in_dim
        #     if l == 0:
        #         in_dim = (input_ch+input_feat)
        #     elif l-1 in W_up:
        #         in_dim = W*2
        #     elif l in self.skips and input_feat>0: #todo
        #         in_dim = W + (input_ch + input_feat)
        #     else:
        #         in_dim = W
        #
        #     if l != D:
        #         # NOTE: beta=100 is important! Otherwise, the initial output would all be > 10, and there is not initial sphere.
        #         layer = SirenLayer(in_dim, out_dim, is_first=(l == 0)) if use_siren \
        #             else DenseLayer(in_dim, out_dim, activation=nn.Softplus(beta=100))
        #     else:
...
class RadianceNet(nn.Module):
    def __init__(self,
        ...
        #     if l != D:
        #         layer = SirenLayer(in_dim, out_dim, is_first=(l == 0)) if use_siren \
        #             else DenseLayer(in_dim, out_dim, activation=nn.ReLU(inplace=True))
        #     else:
        #         # Linear (For segmentation or logit case)
        #         layer = nn.Linear(in_dim, out_dim, bias=True) if output_dim > 3 else \
