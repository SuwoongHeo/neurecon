...
class GeomConvLayers(nn.Module):
    ...
    '''
    def __init__(self, input_nc=16, nf=16, output_nc=16, use_relu=False):
        super().__init__()
        self.use_relu = use_relu

        self.conv1 = nn.Conv2d(input_nc, nf, kernel_size=5, stride=1, padding=2, bias=False)
        self.conv2 = nn.Conv2d(nf, nf, kernel_size=5, stride=1, padding=2, bias=False)
        self.conv3 = nn.Conv2d(nf, output_nc, kernel_size=5, stride=1, padding=2, bias=False)
        if use_relu:
            self.relu = nn.LeakyReLU(0.2, inplace=True)
    ...
    def forward(self, x):
        ...
        if self.use_relu:
            x = self.relu(x)
        x = self.conv3(x)
        #todo tmp, residual connection
        x = x + x_in

        return x

#todo Hourglass backbone, Diffusion net backbone
class GeomConvBottleneckLayers(nn.Module):
    ...
    def __init__(self, input_nc=16, nf=16, output_nc=16, use_relu=False):
        ...
        self.conv1 = nn.Conv2d(input_nc, nf, kernel_size=4, stride=2, padding=1, bias=False)
        ...
        self.up1 = nn.ConvTranspose2d(nf*4, nf*2, kernel_size=4, stride=2, padding=1, bias=False)
        self.up2 = nn.ConvTranspose2d(nf*2, nf, kernel_size=4, stride=2, padding=1, bias=False)
        self.up3 = nn.ConvTranspose2d(nf, output_nc, kernel_size=4, stride=2, padding=1, bias=False)
...
class UnetNoCond6DS(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, nf=64, up_mode='upconv', use_dropout=False, return_lowres=False, return_2branches=False):
        ...
        self.return_lowres = return_lowres
        self.return_2branches = return_2branches
        # input_nc, output_nc, kernel_size, stride, padding, use_bias=False, use_bn=True, use_relu=True
        self.conv1 = Conv2DBlock(input_nc, nf, 4, 2, 1, use_bias=False, use_bn=False, use_relu=False)
        self.conv2 = Conv2DBlock(1 * nf, 2 * nf, 4, 2, 1, use_bias=False, use_bn=True)
        self.conv3 = Conv2DBlock(2 * nf, 4 * nf, 4, 2, 1, use_bias=False, use_bn=True)
        self.conv4 = Conv2DBlock(4 * nf, 8 * nf, 4, 2, 1, use_bias=False, use_bn=True)
...
def get_unet_backbone(name, backbone_config):
    assert name != 'GaussianSmoothingLayers', 'The layer GaussianSmoothingLayers is not used currently'
    if name != 'none':
