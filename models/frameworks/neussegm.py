...
class NeuSSegm(nn.Module):
    ...
    def forward_segm(self, x: torch.Tensor, view_dirs: torch.Tensor):
        _, nablas, geometry_feature = self.implicit_surface.forward_with_nablas(x)
        logits = self.segm_net.forward(x, view_dirs, nablas, geometry_feature) #Note, View_dirs, nablas, are not used here
        return logits
    ...
    **dummy_kwargs  # just place holder
    ...
    def render_rayschunk(rays_o: torch.Tensor, rays_d: torch.Tensor):
        ...
        if N_outside > 0:
            ...
            sigma_out, radiance_out, logits_out = batchify_query(model.nerf_outside.forward, x_out, views_out)
        ...
        if N_outside > 0:
            ...
            logits_in = logits * mask_inside.float()[...,None] + logits_out[..., :N_pts_1,:] * (~mask_inside).float()[...,None]
...
class SingleRenderer(nn.Module):
    def __init__(self, model: NeuSSegm                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            ):
        super().__init__()
        self.model = model
...
class Trainer(nn.Module):
    def __init__(self, model: NeuSSegm, device_ids=[0], batched=True):
        super().__init__()
        self.model = model
    ...
    def forward(self, 
        ...
        object_mask = model_input['object_mask'].view((render_kwargs_train['H'], render_kwargs_train['W'], -1)).to(device)
        rays_o, rays_d, select_inds = rend_util.get_rays(
            ...
            mask=object_mask if args.training.get('sample_maskonly') is not None else None)
        # [B, N_rays, 3]
        ...
        # [B, N_rays,]
        losses['loss_seg'] = args.training.w_seg * F.cross_entropy(extras['logit'].view((-1,4)), target_segm.view((-1,)))
        #F.cross_entropy()
...
def get_model(args):
    ...
    segmentation_cfg = {
        'use_siren': args.model.segmentation.setdefault('use_siren', args.model.setdefault('use_siren', False)),
        'embed_multires': args.model.segmentation.setdefault('embed_multires', -1),
        'embed_multires_view': args.model.segmentation.setdefault('embed_multires_view', -1),
        'use_view_dirs': args.model.segmentation.setdefault('use_view_dirs', False),
        'D': args.model.segmentation.setdefault('D', 5),
        'W': args.model.segmentation.setdefault('W', 256),
        'skips': args.model.segmentation.setdefault('skips', []),
        'output_dim': args.model.segmentation.setdefault('output_dim', 4)
    }
    
    model_config['surface_cfg'] = surface_cfg
    model_config['radiance_cfg'] = radiance_cfg
    model_config['segmentation_cfg'] = segmentation_cfg

    model = NeuSSegm(**model_config)
