...
def main_function(args):
    args.update({'device_ids': [3]})  # Temporary
    init_env(args)
    ...
    # ---------------------------------------------
    # todo, implement pretrain hook to fit it to the SMPL
    if is_master():
    ...
    log.info('=> Start training..., it={}, lr={}, lr_feat{}, in {}'.format(it, optimizer.param_groups[0]['lr'],
                                                                           optimizer.param_groups[1]['lr'], exp_dir))
    end = (it >= args.training.num_iters)
    with tqdm(range(args.training.num_iters), disable=not is_master()) as pbar:
        if is_master():
            pbar.update(it)
        while epoch_idx <= args.training.num_epochs and not end:  # todo, add num_epochs to training config
            try:
                if args.ddp:
                    train_sampler.set_epoch(epoch_idx)

                # Validates at every epoch
                with torch.no_grad():
                    # -------------------
                    # validate
                    # -------------------
                    ...
                    val_imgs = trainer.val(val_in, val_gt, render_kwargs_test, device=device)
                    for name, img in val_imgs.items():
                        logger.add_imgs(img, name, epoch_idx, prefix=val_in['tag'][0])
                    # -------------------
                    # validate mesh
                    # -------------------
                    if is_master():
                        ...
                        # todo check validation interval?
                        cbuvmap = val_in['cbuvmap'].to(device)
                        idfeatmap = render_kwargs_test['idfeat_map_all'][val_in['subject_id'].item()][None].to(
                            device)
                        smplparams = val_in['smpl_params']['poses'][:, 0, :].to(device)
                        with torch.no_grad():
                            cbfeat, idGfeat, _, _ = model.forward_featext(cbuvmap, idfeatmap)
                            io_util.cond_mkdir(mesh_dir)
                            meshfunc = functools.partial(model.forward_sdf, cbfeat=cbfeat, idGfeat=idGfeat,
                                                         smpl_param=smplparams)
                            if hasattr(model, 'decoder'):
                                mesh_util.extract_mesh(
                                    model.implicit_surface,
                                    filepath=os.path.join(mesh_dir,
                                                          '{:08d}_{}.ply'.format(epoch_idx, val_in['tag'][0])),
                ...
                for (indices, model_input, ground_truth) in dataloader:
                    ...
                    if i_val > 0 and int_it % i_val == 0:
                        with torch.no_grad():
                            ...
                            val_imgs = trainer.val(val_in, val_gt, render_kwargs_test, device=device)
                            for name, img in val_imgs.items():
                                logger.add_imgs(img, name, it, prefix=val_in['tag'][0])
                    ...
                    if is_master():
                        ...
                        # ----------------------------------------------------------------------------
                        pbar.set_postfix(lr=optimizer.param_groups[0]['lr'], lr_feat=optimizer.param_groups[1]['lr'],
